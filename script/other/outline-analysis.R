...
ggsave(GM_shape_variation$gg,
       filename = "output/figures/Figure_S2.tif",
...
ggsave("plot_momocs.tiff", plot.momocs, dpi = 300, width = 10, height = 8, units = "in")
...
tiff("output/figures/Figure_S4.tif", width = 8, height = 6, units = "in", res = 300)  # Adjust resolution and dimensions as needed

# Create the PCA plot
plot.momocs <- plot_PCA(GM_perforation_centered_scaled.PCA, axes = c(1, 2), ~ Type, 
                        morphospace = TRUE, points = TRUE, zoom = 0.9, chull = FALSE, 
                        palette = pal_jama(), legend = TRUE) %>% 
