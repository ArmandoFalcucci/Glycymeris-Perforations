...
GM_perforation <- Out(Coordinates_2DGM, fac = select(dataset.filtered, c("Type")))
...
# 6. EFA analysis ----
GM_perforation_centered_scaled.EFA <- efourier(GM_perforation_centered_scaled, nb.h = 53, smooth.it = 0, norm = T)
...
GM_PCScores <- as.data.frame(GM_perforation_centered_scaled_rotated.PCA$x)
