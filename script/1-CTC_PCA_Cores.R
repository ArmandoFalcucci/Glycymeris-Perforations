...
PCA_shell <- dataset.filtered %>%
  ...
  rename(`Area` = `Area perforation m2`, Circumference = `Circumference perf.`) %>%
  na.omit()
...
fviz_eig(PCA_shells.morpho.size, addlabels = T,
                             repel=T,
...
PCA1.2a <- factoextra::fviz_pca_ind(PCA_shells.morpho.size,
